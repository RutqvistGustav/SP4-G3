...
void StyleColorsDark()
{
    ...
    GImNodes->Style.Colors[ImNodesCol_NodeBackgroundSelected] = IM_COL32(95, 95, 95, 255);
    ...
    GImNodes->Style.Colors[ImNodesCol_TitleBarSelected] = IM_COL32(75, 159, 255, 255);
    GImNodes->Style.Colors[ImNodesCol_StartNodeTitleBar] = IM_COL32(235, 50, 50, 255);
    GImNodes->Style.Colors[ImNodesCol_StartNodeTitleBarHovered] = IM_COL32(255, 20, 20, 255);
    GImNodes->Style.Colors[ImNodesCol_StartNodeTitleBarSelected] = IM_COL32(255, 0, 0, 255);
...
void BeginNode(const int node_id, const bool start_node)
{
    ...
    if (start_node)
    {
        node.ColorStyle.Titlebar = GImNodes->Style.Colors[ImNodesCol_StartNodeTitleBar];
        node.ColorStyle.TitlebarHovered = GImNodes->Style.Colors[ImNodesCol_StartNodeTitleBarHovered];
        node.ColorStyle.TitlebarSelected = GImNodes->Style.Colors[ImNodesCol_StartNodeTitleBarSelected];
    }
    else
    {
...
void SetNodeOrigin(int node_id, int new_node_id)
{
    ImNodesEditorContext& editor = EditorContextGet();
    ImNodeData& node = ObjectPoolFindOrCreateObject(editor.Nodes, node_id);
    ImNodeData& newNode = ObjectPoolFindOrCreateObject(editor.Nodes, node_id);
...
void NodeLineHandler(ImNodesEditorContext& editor, const char* const line)
{
 ...
 /*   else if (sscanf_s(line, "[node. % b", &isStart) == 2)
    {
        ImNodeData& node = editor.Nodes.Pool[GImNodes->CurrentNodeIdx];
