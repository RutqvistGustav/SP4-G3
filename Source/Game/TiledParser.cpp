...
bool TiledParser::Load(const std::string& aMapPath)
{
	...
	tson::Layer* backgroundWallWindowLayer = map->getLayer("Windows");
	...
	const std::array<std::pair<tson::Layer*, int>, 6> layers = {
		{
			{ backgroundWallWindowLayer, GameLayer::BackgroundWallWindow },
...
bool TiledParser::ParseEntityLayer(tson::Layer* aLayer)
{
	...
	for (auto& object : aLayer->getObjects())
	{
		...
		const CU::Vector2<float> size = CU::Vector2<float>(static_cast<float>(object.getSize().x), static_cast<float>(object.getSize().y));
		CU::Vector2<float> position = CU::Vector2<float>(static_cast<float>(object.getPosition().x), static_cast<float>(object.getPosition().y));
		...
		myResult->AddEntity(TiledEntity(position, size, properties));
	}
